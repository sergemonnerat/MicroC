                              ...
                              OS_ERR    *p_err)
{
    ...
    while (p_pend_list->NbrEntries > (OS_OBJ_QTY)0u) {
        ...
        new_prio    = p_tcb_owner->Prio;
        if ((p_tcb_owner       != (OS_TCB *)0) &&               /* Has the owner inherited a priority?                  */
            (p_tcb_owner->Prio !=  p_mutex->OwnerOriginalPrio)) {
            if(p_pend_list->HeadPtr->NextPtr != (OS_PEND_DATA*)0u) {
                new_prio = p_pend_list->HeadPtr->NextPtr->TCBPtr->Prio;
            } else {
                new_prio = p_mutex->OwnerOriginalPrio;
            }
        }

        if(new_prio != p_tcb_owner->Prio) {
            switch (p_tcb_owner->TaskState) {                   /* yes                                                  */
                case OS_TASK_STATE_RDY:
                     OS_RdyListRemove(p_tcb_owner);
